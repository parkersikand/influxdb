...
// TruncateShardGroup marks a shard group as not accepting writes after time t
func (data *Data) TruncateShardGroup(database, policy string, id uint64, t int64) error {
	rpi, err := data.RetentionPolicy(database, policy)
	if err != nil {
		return err
	}
	...
	for i := range rpi.ShardGroups {
		if rpi.ShardGroups[i].ID == id {
			rpi.ShardGroups[i].TruncatedAt = time.Unix(0, t)
			return nil
		}
	}
